...
class DB:
    ...
    def initiate(self):
        encoder_path = f'{dirname(dirname(__file__))}/static/encoder.pt'
        decoder_path = f'{dirname(dirname(__file__))}/static/decoder.pt'
        ...
        dummy_hidden = rai.BlobTensor.from_numpy(np.zeros((4, 1, 500)))
    ...
    def process(self, nparray):
        ...
        self.con.modelrun('encoder', input=['sentence', 'hidden'], output=['e_output', 'd_hidden'])
